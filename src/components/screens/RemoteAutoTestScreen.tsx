...
export default function RemoteAutoTestScreen() {
  ...
  const handleOpenCsv = useCallback(async () => {
    ...
    try {
      ...
      const savedCheckpoint = loadAutoTestCheckpoint('remote', nextFilePath, nextRows.length)
  ...
  const pauseAtCheckpoint = useCallback((nextIndex: number) => {
    saveAutoTestCheckpoint('remote', filePath, nextIndex)
  ...
  const runFrom = useCallback(async (startIndex: number, restart: boolean) => {
    if (runningRef.current || rows.length === 0 || !host.trim()) return
    ...
    if (restart) {
      clearAutoTestCheckpoint('remote', filePath)
      setCheckpointIndex(null)
      setStatusText('Remote auto test restarted.')
    } else {
    ...
    runningRef.current = false
    setRunning(false)
  }, [filePath, host, markRowDone, pauseAtCheckpoint, port, rows, running, waitForInterval])
